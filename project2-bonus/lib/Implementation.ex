defmodule GossipSimulator.Implementation do
    def gossip_full(numNodes,nodes_to_fail,trigger_node_count,nodes_to_ping,stopping_threshold) do
        for i <- 1..numNodes do
        ...
        gossip_failure(numNodes,nodes_to_fail,0)
    ...
    def gossip_2d(numNodes,nodes_to_fail,trigger_node_count,nodes_to_ping,stopping_threshold) do
    ...
    def gossip_line(numNodes,nodes_to_fail,trigger_node_count,nodes_to_ping,stopping_threshold) do
        for i <- 1..numNodes do
            neighboursList =  cond do
                                    i == 1 -> [i+1]
                                    i == numNodes -> [i-1]
    ...
    def gossip_imp2d(numNodes,nodes_to_fail,trigger_node_count,nodes_to_ping,stopping_threshold) do
        rowcnt = round(:math.sqrt(numNodes))
        for i <- 1..numNodes do
            neighboursList =  cond do
                                    i == 1 -> [i+1,i+rowcnt]
    ...
    def gossip_failure(numNodes,nodes_to_fail,nodes_failed) do
        if nodes_failed < nodes_to_fail do
            fail_node = :rand.uniform(numNodes)
            fail_node_id = GossipSimulator.Gossip_Default_Actor.whereis(fail_node)
            if fail_node_id != nil do
                Process.exit(fail_node_id,:kill)
                gossip_failure(numNodes,nodes_to_fail,nodes_failed+1)
            else
                gossip_failure(numNodes,nodes_to_fail,nodes_failed)
            end
        end
    end

    def pushsum_full(numNodes,nodes_to_fail,trigger_node_count,nodes_to_ping,stopping_threshold) do
        for i <- 1..numNodes do
        ...
        :global.register_name(:mainproc,convergence_task.pid)
        :global.register_name(:failurehelper,self())
        pushsum_failure(numNodes,nodes_to_fail,0)
        start_time = System.system_time(:millisecond)
    ...
    def pushsum_2d(numNodes,nodes_to_fail,trigger_node_count,nodes_to_ping,stopping_threshold) do
        rowcnt = round(:math.sqrt(numNodes))
        for i <- 1..numNodes do 
            neighboursList =  cond do
                                    i == 1 -> [i+1,i+rowcnt]
    ...
    def pushsum_line(numNodes,nodes_to_fail,trigger_node_count,nodes_to_ping,stopping_threshold) do
        for i <- 1..numNodes do
            neighboursList =  cond do
                                    i == 1 -> [i+1]
                                    i == numNodes -> [i-1]
    ...
    def pushsum_imp2d(numNodes,nodes_to_fail,trigger_node_count,nodes_to_ping,stopping_threshold) do
        rowcnt = round(:math.sqrt(numNodes))
        for i <- 1..numNodes do
            neighboursList =  cond do
                                    i == 1 -> [i+1,i+rowcnt]
    ...
    def pushsum_failure(numNodes,nodes_to_fail,nodes_failed) do
        if nodes_failed < nodes_to_fail do
            fail_node = :rand.uniform(numNodes)
            fail_node_id = GossipSimulator.PushSum_Default_Actor.whereis(fail_node)
            if fail_node_id != nil do
                Process.exit(fail_node_id,:kill)
                pushsum_failure(numNodes,nodes_to_fail,nodes_failed+1)
            else
                pushsum_failure(numNodes,nodes_to_fail,nodes_failed)
            end
        end
    end
