defmodule GossipSimulator.PushSum_Default_Actor do
    use GenServer
    require Logger
    ...
    def init([node_id,neighboursList,nodes_to_ping]) do
        receive do
            {_,s,w} -> rumoringProcess = Task.start fn -> start_gossiping(node_id,neighboursList,s+node_id,w+1,nodes_to_ping) end
    ...
    def start_gossiping(node_id,neighboursList,s,w,nodes_to_ping) do
        try do
            {s,w} = receive do
            ...
            start_gossiping(node_id,neighboursList,s,w,nodes_to_ping)
        rescue 
            _ -> start_gossiping(node_id,neighboursList,s,w,nodes_to_ping)
        end
    end
end